...
class StepGenerator {
    ...
    async generateStep(stepId, memory, options = {}) {
        ...
        for (let index = 0; index < candidateCount; index += 1) {
            const variantPrompt = candidateCount > 1
                ? `${prompt}\n\n请生成一个与前面候选方案明显不同、但仍然自洽的新方案。这是第 ${index + 1} 个候选。`
                : prompt;
    ...
    buildStepPrompt(stepId, context, template) {
        ...
        const sections = [
            '你是一名专业中文 RPG 游戏设计师。',
            '所有说明性内容必须用中文输出，语气自然，不要输出英文说明。',
            'JSON 键名请严格遵守给定结构，便于前端直接解析。',
            `当前步骤：${definition.name}`,
            `游戏类型：${context.gameType}`,
            template?.name ? `参考模板：${template.name}` : '',
            template?.description ? `模板特点：${template.description}` : '',
    ...
    getStepDefinition(stepId, template) {
        ...
        const definitions = {
            ...
            mainPlot: {
                ...
                instructions: [
                    '请只返回一个 JSON 对象。',
                    '{"title":"主线标题","summary":"剧情概要","theme":"主题","incitingIncident":"引发事件","chapters":[{"id":"chapter_1","name":"章节名","description":"章节内容","goal":"章节目标","characters":["角色ID"],"locations":["地点"],"encounters":[{"type":"战斗/对话/谜题/事件","description":"遭遇说明"}],"keyDecisions":["关键抉择"],"rewards":["奖励"],"failCondition":"失败条件","ending":"章节结尾","leadsTo":"下一章节"}],"climax":"高潮","resolution":"结局说明","multipleEndings":[{"condition":"达成条件","description":"结局描述"}]}',
            ...
            sidePlots: {
                ...
                instructions: [
                    ...
                    '[{"id":"side_1","name":"支线名","type":"人物支线/探索支线/收集任务/历史揭秘/道德抉择","description":"支线内容","trigger":"触发条件","characters":["角色ID"],"locations":["地点"],"steps":[{"description":"步骤说明","type":"探索/战斗/对话/谜题"}],"rewards":["奖励"],"relationToMainPlot":"与主线的联系","optional":true,"ending":"支线结局"}]',
                    '要求：每条支线都要强化主线或角色弧光。'
                ].join('\n')
            },
            fragments: {
